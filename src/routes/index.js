...
router.get('/imgTracking/:imgtype/:refcode/:emailType/', async (req, res) =>{
    let encodedImgUrl = req.url
    let imgArray = encodedImgUrl.split('/');
    let studentCode = imgArray[3],
        emailType = imgArray[4],
        eventType = "Image_URL_Request",
        time = date.format(new Date(), 'YYYY-MM-DD HH:mm:ss');

    try {
        let user = await connection.query('SELECT email FROM users WHERE `studentId`=(?)', [studentCode]);
        if (user.length !== 0) {
